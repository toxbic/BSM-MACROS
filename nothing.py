        ...
        if match:
            ...
            if found_stat_name:
                try:
                    ...
                    if val > 100 and max_r < 100:
                        val = val // 10
                    if max_r < 20 and val > 20:
                        val = val // 10
                    if found_stat_name == "Convert Rate" and val > 10:
                        val = val / 100
...
def run_macro(gui_data, log_main, log_raw):
    ...
    session_dir = os.path.join("Session_Logs", f"Session_{timestamp}")
    try:
        ...
        with open(html_path, "w", encoding="utf-8") as f:
            f.write(
                ...
                ".passives{color:#d32f2f;font-weight:bold}"
                ".stats{color:#333;font-size:0.9em}"
                "img{border:1px solid #ccc}"
                ".credits{font-size:0.8em;color:#555;margin-bottom:10px}"
                "</style></head>"
                "<body><h2>SSA Roll Session Log</h2>"
                "<div class='credits'>Made by TOXBIC</div>"
                "<table><tr><th style='width:50px'>Roll</th>"
                "<th>Detected Stats</th><th>Screenshot</th></tr>"
            )
    except Exception as e:
        log_main(f"Log Error: {e}")
...
class MacroGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("SSA Macro Multi-Target – TOXBIC")
        try:
